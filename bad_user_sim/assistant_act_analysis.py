...
# ---------------------------------------------------------------------------
def main(skip_participant_adjust=False):
    """skip_participant_adjust drops the two sections that condition the assistant's
    effect on what the PARTICIPANT did (section 4, the Think-Aloud-controlled ordinal
    models, and section 7, assistant act -> participant Think Aloud).

    They answer a different question from "which assistant acts go with better
    transfer": they ask how much of an assistant act's association is really the
    student's own disposition. That adjustment is worth making on a human population,
    where participants differ in ability for reasons outside the design. In the
    simulator the assistant is one fixed model whose behaviour is driven by the
    student model, so the same adjustment removes the variation that stands in for
    participant ability -- and the clean answer there is the randomised explain/no_exp
    arms, not a covariate.
    """
    ...
    if not skip_participant_adjust:
...
if __name__ == "__main__":
    ...
    _ap.add_argument("--skip-participant-adjust", action="store_true",
                     help="drop sections 4 and 7, which condition on the participant's acts")
    main(**{"skip_participant_adjust": _ap.parse_args().skip_participant_adjust})
