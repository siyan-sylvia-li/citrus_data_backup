"""Assistant-side dialogue-act annotation (TUTOR moves from Table I). OTHELLO.

Same scheme, panel and vote as the Connect Four phase's copy of this module
(game_user_study_phase_1/dialogue_act_annotation_assistant.py) — only the worked
examples and the signature's board vocabulary are Othello's. Keeping the act list
identical is deliberate: the two phases' assistant-act rates are only comparable if
they were coded against the same taxonomy.
...
TUTOR moves, so a turn of `conversation_poc20260727.jsonl` ends up with a student-act
label set and a tutor-act label set that can be cross-tabulated.
...
SETTING_EXAMPLES = {
    "Provide Correct Answer": '"Play **c8**." "I\'d suggest b7 here."',
    "Direct Instruction": '"a7 is directly next to the empty corner a8, so it likely hands White that corner." "In the endgame the winner is whoever has more discs when the board runs out, so flipping the most discs now is usually the wrong instinct."',
    "Hint": '"The key danger is the empty corner **a8**." "Look at which of your legal squares sit next to a corner."',
    "Prompt": '"After you play c8, what legal moves does White get?"',
    "Pose Simplified Problem": '"Which move leaves White the fewest good replies?"',
    "Paraphrase": '"Your a7 idea would flip the two discs along the a-file."',
    "Positive Feedback": '"Good — you spotted that b8 gives up the corner."',
    "Negative Feedback": '"b7 isn\'t legal in this position, so that one is out."',
    "Neutral Feedback": '"Edge squares are often safer than middle squares if they\'re stable, but you still have to check corners and mobility."',
}
...
class AssistantDialogueActClassifierSignature(dspy.Signature):
    """Given a reply from an AI assistant that is helping a user solve an Othello (Reversi) endgame puzzle, first split it into clauses, then classify each clause into the tutor dialogue acts that apply per the taxonomy provided. Output all applicable acts. Assistant replies are long and usually carry several acts; do not stop at the first one.

Disambiguating the acts that are easily confused — read carefully:
- "Provide Correct Answer": names a concrete square for the user to play ("play c8", "I'd suggest b7"). Only for the MOVE ITSELF, not for other factual content.
- "Direct Instruction": asserts content the user did not supply — a rule of the game, a strategic principle, or a reading of the board ("a7 hands White the corner a8", "corners can never be flipped back"). It RESOLVES the question rather than handing it back.
- "Hint": points the user toward where the answer lies without resolving it ("the key danger is the empty corner a8", "look at which legal squares sit next to a corner"). If the text states the conclusion, it is Direct Instruction, not a Hint.
- "Prompt": a question that cues one specific next step the user should take or check ("after you play c8, what legal moves does White get?"). Rhetorical questions the assistant then answers itself are still Prompts only if the user is expected to work it out.
- "Pose Simplified Problem": hands back an easier, self-contained sub-question that the user is meant to solve ("which move leaves White the fewest good replies?"). Prompt cues the next step in the CURRENT line of reasoning; Pose Simplified Problem substitutes a smaller problem for it.
...
- "Paraphrase": restates the user's own proposal, move, or reasoning back to them ("your a7 idea would flip the two discs along the a-file"). Restating the USER; adding new board facts is Direct Instruction.
- "Positive Feedback": affirms the user's idea or move as correct/good ("good — you spotted that b8 gives up the corner", "that's right").
- "Negative Feedback": marks the user's idea or move as wrong ("b7 isn't legal there", "no, that would give up the corner").
- "Neutral Feedback": neither endorses nor rejects — partial credit or a hedge ("edge squares are often safer, but you still have to check corners", "it's tempting, though consider...").
"""
...
if __name__ == "__main__":
    ...
    samples = [
        "Play **c8**.\n\nWhy: it keeps White off the empty corner a8 and leaves White very few legal replies.",
        "Edge squares are often safer than middle squares if they're stable, but ask yourself: after you play a7, what legal moves does White get?",
    ]
    for u in samples:
        r = suite(utterance=u)
