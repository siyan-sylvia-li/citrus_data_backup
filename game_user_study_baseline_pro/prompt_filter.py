...
class JudgeSuite(dspy.Module):
    def __init__(self, callbacks=None):
        ...
        # Two-judge panel: Llama-3.3-70B (Together, Predict) + gpt-5.4-mini
        # (OpenAI, ChainOfThought — reasoning improves its calibration and it's
        # fast enough that CoT stays cheap). Both are fast + serverless.
        # (Qwen was dropped: Together moved all sufficiently-strong Qwen models
        # off serverless, and the fast small ones / big reasoning ones are either
        # too weak or too slow for a real-time gate. See git history / notes.)
        # timeout caps a slow/straggling model; on timeout that grader errors,
        # is dropped, and the panel returns from the other (fail-open).
        specs = {
