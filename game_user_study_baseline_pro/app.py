...
CF_ROUNDS = [
    {"puzzle": "15",    "ai": False,  "ai_survey": False,  "gate": True,
     ...
     "intro": {"title": "Puzzle 1",
               "body": "You will have <strong>five minutes</strong> to play the first three moves in this win-in-five-moves puzzle."}},
    ...
    {"puzzle": "w4p6", "ai": False, "ai_survey": False,
     ...
     "intro": {"title": "Puzzle 2",
               "body": "You will have <strong>three minutes</strong> to play the first three moves in this win-in-four-moves puzzle."}},
]
...
def study():
    ...
    return render_template(
        ...
        # include chat DOM+JS (hidden until unlocked) only if this round ever has AI;
        # a gated round with ai=False screens/reset the board but never shows the assistant
...
def cf_gate():
    ...
    return jsonify({
        "ok": True,
        "unlocked": True,
        "ai": ai_enabled(),            # False for a gated no-AI round -> client won't reveal chat
        "board": board,
        "moves_made": moves_made,
        "num_moves": CF_NUM_MOVES,
        "moves_left": CF_NUM_MOVES - moves_made,
