...
class DialogueActClassifierSignature(dspy.Signature):
    """Given an utterance from a user talking to an AI assistant while solving an Othello endgame puzzle, first split it into clauses, then classify each clause into the dialogue acts that apply per the taxonomy provided. Output all applicable acts.
...
- "Common Ground Question": the user proposes a SPECIFIC candidate move (or their own read of the position) and asks the assistant to confirm/evaluate it. E.g. "is b7 good?", "was my last move good?", "would a8 be better?".
- "Knowledge Deficit Question": asks about a concept, rule, or the board (e.g. threats), or clarifies something the assistant said — NOT a request for the move itself. E.g. "why not c8?", "are there any threats?", "what do you mean by that?".
"""
