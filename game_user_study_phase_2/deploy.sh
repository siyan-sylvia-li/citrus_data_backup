#!/usr/bin/env bash
# Deploy the CITRUS pre-filter study (prompt task -> LLM-judge filter -> Othello
# + AI chat) to Cloud Run, with participant data persisted to a GCS bucket
...
SERVICE="${SERVICE:-game-study-phase-2}"
...
INTERVENTION_VERSION="${INTERVENTION_VERSION:-v9_two_rows}"
...
# app.py as OTH_ROUNDS; it is structural, not an env var.
...
OTH_TIME_LIMIT_SECONDS_2="${OTH_TIME_LIMIT_TRANSFER:-90}"  # round 2 countdown (s); 180 = 3 min
