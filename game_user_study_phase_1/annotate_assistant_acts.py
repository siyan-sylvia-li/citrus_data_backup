"""Annotate every Round-1 ASSISTANT reply with the tutor-move panel.

Reads `annotated_conversation_p15.jsonl` when it exists (so the participant-side
`annotation_user` written by the notebook is preserved) and otherwise falls back to
the raw `conversation_p15.jsonl`. Adds an `annotation_assistant` field per turn and
