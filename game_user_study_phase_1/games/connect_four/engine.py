...
def grid_text(board: Board) -> str:
    """The labelled grid used in prompts AND in follow-up chat turns.

    One renderer for both, deliberately: switching board formats midway through
    a conversation is a needless source of model error.
    """
    lines = ["     " + "  ".join(str(c + 1) for c in range(COLS))]
    for r in range(ROWS - 1, -1, -1):                 # top row first, visually
        lines.append(f"  {r + 1}  " + "  ".join(_PIECE_TO_CHAR[board[r][c]]
                                                for c in range(COLS)))
    return "\n".join(lines)
...
def board_to_prompt(
    ...
    *,
    ...
    evaluation, where the text is all the model gets, so it spells out what a
    reader would otherwise have to infer: which way the rows run, which columns
    are playable, and where a disc would land.

    Mirrors games/othello/engine.board_to_prompt, including why the flags exist —
    each one changes what is being measured:

    legal=True (default)
        Lists the playable columns. Cheap here (a column is playable unless it is
        full), but it removes a silly failure mode where the model picks a full
        column and the run dies for reasons unrelated to Connect Four.
    landing=False
        With landing=True each column is shown with the ROW the disc would land
        on. That is the one piece of Connect Four bookkeeping models reliably get
        wrong, so turning it on isolates strategy from arithmetic.
    """
    me = "Red (R)" if to_move == RED else "Yellow (Y)"
    opp = "Yellow (Y)" if to_move == RED else "Red (R)"
    out = [f"Connect Four. You are {me}; {opp} is your opponent.",
           "",
           "Board — columns are numbered 1-7 left to right, rows 1-6 from the",
           "BOTTOM up, so row 1 is the floor and a disc dropped in a column falls",
           "to the lowest empty row. 'R' is a red disc, 'Y' yellow, '.' empty.",
           ""]
    out.append(grid_text(board))
    out.append("")

    if lists:
        def squares(value):
            return [f"c{c + 1}r{r + 1}" for r in range(ROWS) for c in range(COLS)
                    if board[r][c] == value] or ["none"]
        out += [f"Red discs: " + ", ".join(squares(RED)),
                f"Yellow discs: " + ", ".join(squares(YELLOW)),
                "(read c4r2 as column 4, row 2 counting up from the bottom)",
                ""]

    out += ["Rules that matter here:",
            "- You drop a disc into a column; it lands on the lowest empty row of",
            "  that column. You cannot choose the row.",
            "- The first player to get FOUR of their own discs in a line wins —",
            "  horizontally, vertically, or on either diagonal.",
            "- A full column cannot be played. If the board fills with no line of",
            "  four, the game is a draw.",
            ""]

    cols = valid_columns(board)
    if legal:
        if not cols:
            out.append("The board is full.")
        elif landing:
            out.append("Playable columns, and where your disc would land:")
            for c in cols:
                row = get_next_open_row(board, c)
                out.append(f"  column {c + 1} — lands on row {row + 1}")
        else:
            out.append("Playable columns: " + ", ".join(str(c + 1) for c in cols))
        out.append("")

    out += ["Play the move that gives you the best result with best play by both",
            "sides from here.",
            "Answer with a single column number, 1-7."]
    return "\n".join(out)


def board_to_json(board: Board, to_move: int = RED) -> dict:
    """The position as structured data, for programmatic prompting or grading."""
    return {
        # Rows top-first, matching how the board is drawn.
        "rows_top_first": ["".join(_PIECE_TO_CHAR[board[r][c]] for c in range(COLS))
                           for r in range(ROWS - 1, -1, -1)],
        "row_1_is": "bottom", "column_1_is": "left",
        "red_discs": [f"c{c + 1}r{r + 1}" for r in range(ROWS) for c in range(COLS)
                      if board[r][c] == RED],
        "yellow_discs": [f"c{c + 1}r{r + 1}" for r in range(ROWS) for c in range(COLS)
                         if board[r][c] == YELLOW],
        "to_move": "red" if to_move == RED else "yellow",
        "playable_columns": [c + 1 for c in valid_columns(board)],
        "landing_row": {c + 1: get_next_open_row(board, c) + 1
                        for c in valid_columns(board)},
    }


def parse_llm_move(text: str, board: Board | None = None) -> int | None:
    """Pull a column (0-indexed) out of free-form model output, or None.

    Models answer in prose and markup ("**Column 4**", "I'd play col 4."), so a
    strict parser would score presentation rather than play. Prefers a number
    that follows the word "column", then a playable one, then the last number
    mentioned — a bare digit in the reasoning should not outrank the conclusion.
    """
    import re

    labelled = [int(m.group(1)) - 1
                for m in re.finditer(r"(?:column|col\.?|c)\s*#?\s*([1-7])\b",
                                     text, re.I)]
    bare = [int(m.group(1)) - 1 for m in re.finditer(r"\b([1-7])\b", text)]
    for candidates in (labelled, bare):
        if not candidates:
            continue
        if board is not None:
            playable = [c for c in candidates if is_valid_column(board, c)]
            if playable:
                return playable[-1]
        else:
            return candidates[-1]
    return (labelled or bare or [None])[-1]
