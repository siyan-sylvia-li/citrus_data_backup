...
echo
# One interpreter for both steps: `python` alone is often absent, and score_from_logs.py
# needs the same deps as the annotator.
...
# Participant-side dialogue acts: the manipulation check reads these, and with no transfer
# outcome they ARE the endpoint. Skips turns already annotated, so it is cheap to re-run.
"$PYTHON" annotate_phase2.py

echo
echo "Done. Participant data is in ${DEST}/"
echo "Assistant-side acts are a separate pass (edit REC in the script to point here):"
echo "  cd ../bad_user_sim && ${PYTHON} phase2_assistant_fine.py"
