#!/usr/bin/env bash
# Tear down the intervention-iteration Cloud Run service.
#
...
# Same guard as deploy.sh: this directory is a copy of phase 2, so refuse to act on the
# phase-2 service by accident.
if [ "$SERVICE" = "game-study-phase-2" ]; then
  echo "Refusing to tear down 'game-study-phase-2' from the iteration directory." >&2
  exit 2
fi
