...
# ---- iteration mode ----------------------------------------------------------
# This deployment exists to iterate on the intervention FORM, and the only endpoint that
# needs is the manipulation check: did the dialogue acts move? The two unassisted transfer
# puzzles are therefore dropped by default, which is ~3 minutes and one survey cheaper per
# participant.
#
# There is then NO transfer outcome. oth_score_pb22*/pbg* are never written, so anything
...
# The contrasting-case tables live in intervention_content.py and are selected by
# whichever FORM is active (see interventions/). app.py no longer knows what the
# material is, only that a form renders some.
from intervention_content import INTERVENTION_VARIANTS  # noqa: E402,F401

# Which FORM the scaffolded arm sees. Forms are randomised per participant across
# LIVE_FORMS, so several can be compared in one run; set LIVE_FORMS to a comma-separated
# subset to restrict it, or to a single id to pin one. Unknown names fail at import
# rather than at the participant's first page load.
#
# `skeleton` is excluded by default: it is a copy-me template with placeholder copy and
# must never be assigned to a real participant.
#
# The vanilla arm has NO pre-task step in this deployment -- it goes from the prompt task
# straight to the primer. So vanilla participants have no filler.json and are not
# effort-screened, and the two arms clear a different number of filters.
import interventions  # noqa: E402

INTERVENTION_REGISTRY = interventions.REGISTRY
_DEFAULT_LIVE = [f for f in INTERVENTION_REGISTRY if f != "skeleton"]

# INTERVENTION_FORM pins ONE form and wins over LIVE_FORMS; LIVE_FORMS is the set to
# randomise over. Both accepted because INTERVENTION_FORM is the name in the README and
# the obvious thing to reach for when pinning.
_pinned = os.environ.get("INTERVENTION_FORM", "").strip()
# A pin silently overrides LIVE_FORMS, which is how a three-arm run once collapsed to two
# (INTERVENTION_FORM defaulted back to a form and LIVE_FORMS was ignored). If both are set
# and disagree, refuse rather than quietly dropping arms.
_wanted = [f.strip() for f in
           re.split(r"[,;\s]+", os.environ.get("LIVE_FORMS", "")) if f.strip()]
if _pinned and len(_wanted) > 1:
    raise SystemExit(
        f"INTERVENTION_FORM={_pinned!r} pins one form, but LIVE_FORMS asks for "
        f"{_wanted}. Pass exactly one: unset INTERVENTION_FORM to randomise.")
# Separator is comma, semicolon or whitespace. gcloud's --set-env-vars parses its whole
# argument as a COMMA-delimited dict, so a comma inside a value silently produces an empty
# env block (observed: a revision deployed with zero env vars). Accepting ";" lets deploy.sh
# pass the list without a comma in it.
LIVE_FORMS = ([_pinned] if _pinned else
              [f.strip() for f in
               re.split(r"[,;\s]+", os.environ.get("LIVE_FORMS", ",".join(_DEFAULT_LIVE)))
               if f.strip()])
for _f in LIVE_FORMS:
    interventions.load(_f)          # fail at import, not mid-study

# ARMS is the randomisation unit: ONE draw decides both the condition and, for a
# scaffolded participant, which form. Drawing condition and then form separately would
# give vanilla half the sample and each form a quarter; drawing once over
# ["vanilla", form_a, form_b] splits three ways evenly.
#
# Set INCLUDE_VANILLA=0 for a forms-only run (every participant gets a pre-task step).
INCLUDE_VANILLA = os.environ.get("INCLUDE_VANILLA", "1") == "1"
ARMS = (["vanilla"] if INCLUDE_VANILLA else []) + list(LIVE_FORMS)
print(f"[study] arms in play: {ARMS}"
      f"{' (form pinned via INTERVENTION_FORM)' if _pinned else ''}", flush=True)


def assign_arm():
    """Draw this participant's arm once and remember it.

    One draw over ARMS sets both `condition` and, when the arm is a form, which form.
    Re-drawing per request would show a participant one arm on GET and record another on
    POST, so the result is stored in the session the first time it is needed.
    """
    arm = session.get("arm")
    if arm not in ARMS:
        arm = random.choice(ARMS)
        session["arm"] = arm
        session["condition"] = "vanilla" if arm == "vanilla" else "scaffolded"
        if arm != "vanilla":
            session["intervention_form"] = arm
    return arm


def session_form():
    """The form THIS participant sees, assigned once and remembered."""
    fid = session.get("intervention_form")
    if fid not in LIVE_FORMS:
        assign_arm()
        fid = session.get("intervention_form")
        if fid not in LIVE_FORMS:            # vanilla arm has no form
            fid = random.choice(LIVE_FORMS)
            session["intervention_form"] = fid
    return interventions.load(fid)
...
def eligibility():
    ...
    if request.method == "POST":
        ...
        # Determine the experimental arm. One draw over ARMS (vanilla + each live
        # form), so a three-arm run splits evenly rather than giving vanilla half.
        assign_arm()
...
def _run_form(form, *, arm, record_name, done_key):
    """Render, validate, effort-gate and record ONE intervention form.

    The route knows nothing about what the form contains -- see interventions/__init__.py
    for the contract. Only the scaffolded arm reaches it: vanilla has no pre-task step in
    this deployment.
    """
    gate = require_participant()
    if gate:
        return gate
    if session.get("condition") != arm:
        return redirect(url_for("primer"))
    if not session.get("prompt_passed"):
        return redirect(url_for("prompt_task"))

    def page(prefill, error):
        # Route defaults first, then the form's own context -- a form that needs a
        # different min_chars (or its own prefill handling) overrides rather than clashes.
        ctx = {"min_chars": EFFORT_MIN_CHARS, "prefill": prefill, "error": error}
        ctx.update(form.context())
        return render_template(form.TEMPLATE, **ctx)

    # Server-side clock for forms that gate on time on page. Stamped when the page is
    # served and read back on POST, so the check does not rely on a client value the
    # participant can edit. Keyed by form id: swapping forms mid-session cannot inherit
    # another form's timer.
    stamp_key = f"form_shown_{form.ID}"

    if request.method != "POST":
        session[stamp_key] = datetime.now(timezone.utc).isoformat()
        return page("", None)

    shown = session.get(stamp_key)
    server_dwell = ((datetime.now(timezone.utc)
                     - datetime.fromisoformat(shown)).total_seconds() if shown else 0.0)
    posted = request.form.to_dict()
    posted["server_dwell"] = server_dwell

    text = (request.form.get(form.TEXT_FIELD) or "").strip() if form.TEXT_FIELD else ""
    record, error = form.validate(posted, EFFORT_MIN_CHARS)
    if error:
        return page(text, error)

    # A form with no free text cannot be effort-gated. A form that HAS free text but
    # gates on it itself (a pattern check, a puzzle) sets EFFORT_GATE = False and skips
    # the judge, so the two gates cannot both screen the same participant out.
    use_judge = bool(form.TEXT_FIELD) and getattr(form, "EFFORT_GATE", True)
    verdict = judge_observation(text, task=form.JUDGE_TASK) if use_judge else None

    payload = dict(record)
    payload.update(form=form.ID, effort=verdict,
                   submitted_at=datetime.now(timezone.utc).isoformat())
    safe_pid = session.get("safe_pid", "anon")
    with open(participant_dir(safe_pid) / record_name, "w") as f:
        json.dump(payload, f, indent=2)

    if verdict and not verdict["passed"]:
        # Screen-out: no genuine engagement with the pre-task step. Distinct from the
        # prompt screen-out, which is about ability; this one is about effort.
        session["screened"] = "effort"
        return redirect(url_for("screened_thank_you_effort"))

    session[done_key] = True
    if form.FEEDBACK_TEMPLATE:
        return render_template(form.FEEDBACK_TEMPLATE, **form.feedback_context(record))
    return redirect(url_for("primer"))


@app.route("/intervention", methods=["GET", "POST"])
def intervention():
    """Scaffolded arm, whichever form this participant was assigned.

    Kept at this URL and route name so the form templates' url_for() calls and any
    existing links keep working across forms.
    """
    return _run_form(session_form(), arm="scaffolded",
                     record_name="intervention.json", done_key="intervention_done")
