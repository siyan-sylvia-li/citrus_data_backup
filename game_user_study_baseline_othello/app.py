...
# Per-round countdown timers (seconds), env-configurable. Round 1 gets 5 min,
# round 2 gets 1.5 min — the puzzles run to completion, which is 6 and 5 scored
# decisions respectively (see OTH_ROUNDS).
#
# BASELINE NOTE: the AI arm gives round 1 8 minutes. That budget covered two
# things — the ~4 MINUTES engaged participants spend on their opening decision
# (orientation: reading the board, learning the rules) and the time spent asking
# the assistant questions. This arm has no assistant, so the chat share of the
# budget is removed and round 1 is 5 minutes: still comfortably over the observed
# orientation cost plus the 30-65s per subsequent decision. Round 2 needs none of
# that startup — the longest anyone spent on it was 76s — so 90s per transfer
# puzzle is unchanged from the AI arm.
OTH_TIME_LIMIT_SECONDS = int(os.environ.get("OTH_TIME_LIMIT_SECONDS", "300"))       # round 1
...
# NO-AI BASELINE ARM of the Othello phase. Structurally identical to
# game_user_study_phase_othello — same puzzles, same order, same scoring, same
# prompt-writing pre-filter, same confidence gate — with ONE manipulation: the
# in-game chat assistant is never available ("ai": False on every round), so the
# AI-assessment survey is dropped too ("ai_survey": False). Everything that
# reads "ai" downstream (the chat panel, the minimum-exchanges finish gate, the
# external post-survey form) therefore switches itself off; the routes are kept
# so the two arms stay diffable.
#
...
#
# "gate": the participant makes an unaided first move and rates their
# confidence; if that move was optimal AND confidence is high they are screened
# out. It is kept here EXACTLY as in the AI arm so both arms screen the same
# people on the same criterion — the arms must not differ in who they admit.
# In the AI arm surviving the gate also unlocks the assistant; here there is
# nothing to unlock, so surviving it just resumes play (a wrong first move still
# resets the board for a clean attempt, a correct one keeps its progress, which
# keeps the scored attempt comparable across arms). See /api/gate.
#
...
OTH_ROUNDS = [
    {"puzzle": "oc20260727", "ai": False, "ai_survey": False, "gate": True,
     ...
     "intro": {"title": "Puzzle 1",
               "body": "You will have <strong>five minutes</strong> to play this position "
                       "out to the end. Whoever has more discs when neither side can move "
                       "wins."}},
    # Rounds 2-3: the transfer BLOCK — two small puzzles back to back, with no
    # survey in between ("survey": False suppresses it, see /round-done). Two
    # puzzles rather than one because round 2 is the primary
    ...
    {"puzzle": "b220260706", "ai": False, "ai_survey": False, "survey": False,
     ...
     "intro": {"title": "Puzzles 2 and 3",
               "body": "Two more puzzles, back to back. Each is smaller than the first, and "
                       "you will have <strong>90 seconds</strong> for each one."}},
...
# INERT IN THIS ARM, kept so app.py stays diffable against the AI version. In the
# AI arm the assisted round requires this many completed exchanges before the
# participant may leave it (it gates the FINISH BUTTON, not the game). The
# requirement never applies where there is no assistant to talk to, and here
# ai_enabled() is always False, so ai_turn_requirement() below always reports
# required=0 / met=True and nothing is ever blocked.
...
def study():
    ...
    return render_template(
        ...
        # Include the chat DOM+JS (hidden until unlocked) only if this round can
        # ever have an assistant. In this baseline arm no round can, so the gated
        # round still screens and resets the board but never renders a chat panel.
        render_chat=(ai_now or (gate_active and rnd["ai"])),
...
def oth_gate():
    ...
    # Survived the gate. The flag is named for the AI arm, where it also unlocks
    # the assistant; here it only means "the gate is resolved, play may resume".
    ...
    return jsonify({
        "ok": True,
        "unlocked": True,
        "ai": ai_enabled(),        # False in this arm -> the client never reveals a chat panel
        **oth_board_state(board),
        "decisions_made": decisions_made,
    })
...
def chat():
    # UNREACHABLE IN THIS ARM: ai_enabled() is False on every round, so this
    # always 403s. Kept (with its helpers) so app.py diffs cleanly against the
    # AI version — flipping a round's "ai" flag is all it takes to re-enable it.
